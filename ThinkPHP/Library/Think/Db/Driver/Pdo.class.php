...
class Pdo extends Db {
	...
	public function __construct($config = '') {
		$config=array( array (
				'DB_USER' => 'root',
				'DB_PWD' => 111111,
				'DB_DSN' => 'mysql:host=localhost;dbname=gogojp;charset=utf8'
		) );
		if (! empty ( $config )) {
			$this->config = $config;
	...
	public function connect($config = '', $linkNum = 0) {
		if (! isset ( $this->linkID [$linkNum] )) {
			...
			try {
				$this->linkID [$linkNum] = new \PDO ( $config[0] ['DB_DSN'], $config[0] ['DB_USER'], $config[0] ['DB_PWD'], $config[1] ['params'] );
			} catch ( \PDOException $e ) {
				E ( $e->getMessage () );
			}
			// 因为PDO的连接切换可能导致数据库类型不同，因此重新获取下当前的数据库类型
			$this->dbType = $this->_getDsnType ( $config ['DB_DSN'] );
	...
	public function query($str, $bind = array()) {
		...
		if (false === $this->PDOStatement)
			E ( $this->error () );
			// 参数绑定
			/* $this->bindPdoParam($bind); BUG ZHENGRUNQIANG */
