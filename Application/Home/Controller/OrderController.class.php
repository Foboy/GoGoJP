...
$logistics_status = I ( 'logistics_status' );  
        ...
        $result= $Order->updateModel (  $order_no,$user_id,$user_account,$order_time,$order_freight,$order_totalprice,$order_payment,$order_status,$order_status_update_time,$order_receive_address,$order_receive_name,$order_receive_mobile,$order_receive_phone,$order_receive_postcode,$remark,$order_pay_account,$logistics_status );
		$this->ajaxReturn ( $result );
	}
	// 获取单个
	...
	public function searchOrder() {
		...
		if (! isset ( $order_time1 ) or empty ( $order_time1 )) {
			$result->Error = ErrorType::RequestParamsFailed;
			$this->ajaxReturn ( $result );
		}
		if (! isset ( $order_time2 ) or empty ( $order_time2 )) {
			$result->Error = ErrorType::RequestParamsFailed;
			$this->ajaxReturn ( $result );
		}
		if (! isset ( $order_status ) or empty ( $order_status )) {
		...
		$result = $Order->searchByPage ( $order_no,  $user_account, $order_time1, $order_time2, $order_status, $pageIndex, $pageSize );
		$this->ajaxReturn ( $result );
	}
}